const Topstocklist=async (today, db_id, callback)=>{             
    ...
    if(defulatConfig.autosimulation && defulatConfig.simulonlyonestock && defulatConfig.simulonlyonestock != '')
    {
        ResolveCallback(defulatConfig.simulonlyonestock.split(',').map(s => s.trim()),'',callback)
        return;
    }
